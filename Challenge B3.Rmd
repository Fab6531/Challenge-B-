...
In order to merge the whole database, we create a function that will merge the data by their Siren number, and we repeat it 10000 times to get the best database. Since our computers have no enough memory, we put eval = FALSE.
...
So we make a simple merger to have an sample of what we would have get with the previous function 
```{r, echo = TRUE, include = TRUE}
...
ggplot(Merger) +
  ...
  ylab("Number of companies") + xlab("Size range of the companies")
```
